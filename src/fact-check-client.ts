import type { Verdict, FactCheckResult, FactCheckSource } from './types.js';
import { compactWhitespace, normalizeVerdict } from './utils.js';
import { createLogger } from './logger.js';

export { normalizeVerdict };

const log = createLogger('fact-check-client');
...
export function buildQueryVariants(claimText: string): string[] {
...
function candidatesFromApiClaims(claimText: string, apiClaims: ApiClaim[] = []): FactCheckCandidate[] {
  const candidates: FactCheckCandidate[] = [];
...
export interface LookupFactChecksOptions {
  apiKey?: string;
  pageSize?: number;
  signal?: AbortSignal;
}
...
export async function lookupFactChecks(
  claimText: string,
  options: LookupFactChecksOptions = {}
  ...
  try {
    let candidates: FactCheckCandidate[] = [];
    const languageVariants: (string | null)[] = ['en-US', 'en', null];
    ...
    for (const query of queryVariants) {
      for (const languageCode of languageVariants) {
        ...
        const data = await response.json() as { claims?: ApiClaim[] };
        const parsed = candidatesFromApiClaims(normalizedClaim, data.claims ?? []);
        if (parsed.length === 0) continue;

        const next = new Map<string, FactCheckCandidate>();
    ...
    const sources: FactCheckSource[] = rankingPool.slice(0, 3).map((entry) => ({
    ...
    return {
      ...
      verdict: 'unverified',
      confidence: 0,
      summary: `Fact-check lookup error: ${(error as Error).message}`,
      sources: [],
    };
  }
}
